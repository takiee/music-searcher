...
class QQMusicSpider(Spider):
    ...
    name = "qqmusic"#用于区分Spider，名字必须唯一
    ...
    # 歌曲列表的URL：根据singerid获取歌曲num首歌曲
    song_list_url = "https://u.y.qq.com/cgi-bin/musicu.fcg?data=%7B%22comm%22%3A%7B%22ct%22%3A24%2C%22cv%22%3A0%7D%2C%22" \
                    "singerSongList%22%3A%7B%22method%22%3A%22GetSingerSongList%22%2C%22param%22%3A%7B%22order%22%3A1%2C%22" \
                    "singerMid%22%3A%22{singer_mid}%22%2C%22begin%22%3A{begin}%2C%22num%22%3A{num}%7D%2C%22module%22%3A%22" \
                    "musichall.song_list_server%22%7D%7D"
    # 获取歌词，需要指定song_id
    ...
    '''
    爬虫启动时，引擎自动调用该方法，并且只会调用一次，用于生成初始的请求对象Request
    '''
    def start_requests(self):
        for i in range(1, self.settings.get('SINGER_PAGE_NUM') + 1):  # 在配置信息里获取歌手页数
            ...
            #callback回调函数，引擎会将下载好的页面（request对象）发送给该方法，执行数据解析功能
            yield request
            # 内地歌手
            request = Request(
                self.start_urls[0].format(index=self.settings.get('SINGER_PAGE_SIZE') * (i - 1), cur_page=i,
    ...
    '''
    引擎将下载好的页面作为参数传递给parse()方法，parse()方法执行从页面中解析数据的功能
    '''
    def parse_singer(self, response):
        ...
        if singer_list:
            for singer in singer_list:
                ...
                for page in range(0, self.settings.get("SONG_PAGE_NUM")):#歌曲列表
                    # 爬取歌手的歌曲
                    request = Request(
    ...
    def process_lyric(self, lyric):
        ...
        #与pattern向匹配的全部字串，返回形式为数组
        #第一个字符为数字，对数字一次或者无限次扩展
        if re_lyric:  # 以
            lyric = re_lyric[0]
            lyric = lyric.replace("&#32;", " ")
            lyric = lyric.replace("&#40;", "(")
